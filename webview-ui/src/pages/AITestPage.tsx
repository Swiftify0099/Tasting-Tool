...
import {
  ...
  Camera, Route, Brain, BarChart3, FileText, Eye
} from 'lucide-react';
...
export default function AITestPage() {
  ...
  const pushLog = (level: LogEntry['level'], msg: string) =>
    setLogs(prev => { const next = [...prev, { level, msg, ts: new Date().toLocaleTimeString() }]; setTimeout(() => logEndRef.current?.scrollIntoView({ behavior: 'smooth' }), 50); return next; });
  ...
  const start = useCallback(() => {
    ...
    fetch('/api/ai-test/run', {
      ...
      if (!res.ok || !res.body) { pushLog('error', 'Server error — check Runner Server workflow'); setRunning(false); return; }
      const reader = res.body.getReader();
      const decoder = new TextDecoder();
      let buf = '';

      const read = () => reader.read().then(({ done: d, value }) => {
        if (d) { setRunning(false); setDone(prev => !prev && true); return; }
      ...
      read();
    }).catch(e => { pushLog('error', `Failed to connect: ${e.message}`); setRunning(false); });
  }, [url, username, password, goal, maxPages, maxSteps]);
    ...
    <div className="h-full flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {running ? `Testing ${curPage ? new URL(curPage).hostname : '…'} — ${visited} page(s) visited` : done ? 'Testing complete' : 'Configure and launch'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {running && (
            <div className="flex items-center gap-1.5 text-[10px] text-amber-400">
              ...
              LIVE · {actPassed + actFailed} actions
            </div>
          )}
          {running
      ...
      <div className="flex-1 overflow-hidden flex min-h-0">
        ...
        <div className="w-[300px] flex-shrink-0 flex flex-col border-r border-slate-800 overflow-y-auto">
          ...
          <div className="p-4 space-y-3 border-b border-slate-800">
            ...
            {/* Credentials toggle */}
            <button
              className="w-full flex items-center gap-2 text-[11px] text-slate-400 hover:text-white transition-colors"
            ...
            <button
              onClick={start} disabled={!url.trim() || running}
              className="w-full btn-primary justify-center text-sm"
            >
              {running ? <><RefreshCw className="w-4 h-4 animate-spin" /> Running…</> : <><Play className="w-4 h-4" /> Start AI Test</>}
            </button>
          </div>
          ...
          {(running || done) && (
            <div className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-center">
                {[
                  { label: 'Pages', value: visited, icon: <Globe className="w-3 h-3" />, color: 'text-blue-400' },
                  { label: 'Queue',  value: queueSz, icon: <Route className="w-3 h-3" />, color: 'text-slate-400' },
                  { label: '✓ Pass', value: actPassed, icon: <CheckCircle className="w-3 h-3" />, color: 'text-emerald-400' },
                  { label: '✗ Fail', value: actFailed, icon: <XCircle className="w-3 h-3" />, color: 'text-red-400' },
                ].map(({ label, value, icon, color }) => (
                  <div key={label} className="bg-slate-800/50 rounded-lg p-2">
                    <div className={`flex items-center justify-center gap-1 text-[10px] ${color} mb-0.5`}>{icon}{label}</div>
              ...
              <div className="space-y-1">
                {routes.map((r, i) => (
                  <div key={i} className="text-[10px] text-slate-400 font-mono truncate flex items-center gap-1">
                    <span className="text-emerald-500">+</span> {(() => { try { return new URL(r).pathname; } catch { return r; } })()}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 font-mono bg-[#080c12] text-xs space-y-0.5">
            {logs.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                ...
                <p className="text-xs text-slate-600">Enter a URL and click "Start AI Test"</p>
              </div>
            ) : (
              <>
                <div className="text-[10px] text-slate-700 mb-3 pb-2 border-b border-slate-800">
                  $ ai-test --url "{url}" --goal "{goal.slice(0, 40)}…" --max-pages {maxPages}
                </div>
                {logs.map((l, i) => (
        ...
        <div className="w-[320px] flex-shrink-0 flex flex-col overflow-hidden">
          ...
          <div className="flex-shrink-0 border-b border-slate-800">
            ...
            <div className="aspect-video bg-[#080c12] flex items-center justify-center overflow-hidden">
              {activeShot ? (
                <img
                  src={`data:image/jpeg;base64,${activeShot.data}`}
          ...
          <div className="flex-1 overflow-y-auto">
            {report ? (
                    ...
                    <div className="space-y-1.5 max-h-32 overflow-y-auto">
                      {report.issues.map((issue, i) => (
                        <div key={i} className="bg-red-500/5 border border-red-500/15 rounded-lg px-2 py-1.5">
                          <p className="text-[9px] font-mono text-red-300 truncate">{(() => { try { return new URL(issue.page).pathname; } catch { return issue.page; } })()}</p>
              ...
              <div className="flex flex-col items-center justify-center h-full gap-2 p-4 text-center">
                ...
                <p className="text-xs text-slate-600">Report will appear here when testing completes</p>
              </div>
            )}
          </div>
        </div>
      ...
      {activeShot && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 cursor-zoom-out"
          ...
          />
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-slate-900/90 px-3 py-1.5 rounded-full text-xs text-white border border-slate-700">
            {activeShot.label}
          </div>
        </div>
      )}
